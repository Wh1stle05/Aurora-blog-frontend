...
import { SiFastapi, SiPostgresql, SiVite, SiTypescript } from 'react-icons/si';
...
export default function Home({ initialLatestPosts = [], initialStats = null }) {
  const hasInitialData = initialLatestPosts.length > 0 || Boolean(initialStats);
  const [latestPosts, setLatestPosts] = useState(initialLatestPosts);
  const [loading, setLoading] = useState(!hasInitialData);
  const [postsError, setPostsError] = useState(false);
  const [stats, setStats] = useState(initialStats || { posts: 0, views: 0, comments: 0, likes: 0 });
                ...
                <div className={styles.latestPosts} data-state={postsState}>
                  {postsError ? (
                    ...
                    latestPosts.map((post, idx) => (
                      ...
                      >
                        <Link to={getPostHref(post)} className={`glass blur ${styles.homeBlogCard}`}>
